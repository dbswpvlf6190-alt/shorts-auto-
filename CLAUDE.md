...
- **클라우드 루틴 프롬프트(v2용)는 `docs/routine_prompt_v2.md`에 작성 완료했으나 적용 대기**: RemoteTrigger update가 자동 승인 검사에 차단됨. 사용자가 적용(직접 붙여넣기 또는 권한 허용)해야 함. 적용 전까지 루틴은 옛 방식 항목(script.txt+images)을 계속 만든다 — `38_비거주1주택종부세원상복구`도 옛 방식이며 다음 실행 때 옛 방식으로 처리됨.
