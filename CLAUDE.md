...
- **주의(목적지 폴더, 2026-09-03 실제 사고)**: 처음엔 바탕화면 루트에 그냥 떨어뜨렸는데, 사용자가 원래 몇 달째 `바탕화면\05_영상_SNS\틱톡 영상\`이라는 정리 폴더에 수동으로 틱톡 영상을 모아왔던 걸 몰랐음 — 그래서 사용자는 계속 그 폴더만 확인했고 새 파일이 안 보인다고 느낌(실제로는 바탕화면 루트엔 있었음). "틱톡 공유가 안 된다"는 항의를 받고서야 발견함. **해결**: `get_tiktok_delivery_path()`가 그 정리 폴더가 존재하면 그쪽에, 없으면(다른 컴퓨터 등) 바탕화면 루트에 저장하도록 수정. **교훈**: 사용자의 기존 수동 워크플로우/폴더 구조를 미리 확인하지 않고 자동화를 얹으면, 기술적으로는 성공해도 사용자 입장에선 "안 되는 것"으로 보일 수 있음 — 새 자동화를 넣기 전에 관련 폴더에 이미 있는 것들을 먼저 둘러볼 것.
