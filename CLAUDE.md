...
## 틱톡 배송 파일 바탕화면 자동 복사 (2026-08-26 추가, 2026-08-27 버그 수정)
`run_queue.py`가 배송 폴더뿐 아니라 `get_desktop_path()`(레지스트리로 실제 바탕화면 경로 조회, OneDrive 리디렉션 여부와 무관하게 동작)로 찾은 바탕화면에도 틱톡 파일을 캡션+해시태그 붙은 파일명 그대로 복사함(사용자가 매번 안 챙겨줘도 되게). **주의**: 목적지가 OneDrive로 동기화되는 폴더 + 파일명에 이모지가 있으면, `shutil.copy2`는 물론 `shutil.copyfile`도 큰 파일(영상)에서는 Windows 고속복사 내부 경로를 타면서 `cp949` 인코딩 에러가 남 (작은 파일에선 재현 안 됨 — 처음엔 이걸로 오판해서 fix가 두 번 필요했음). **해결**: `open(...).read()`/`open(...).write()`로 직접 청크 복사(`shutil.copyfileobj`)해서 그 내부 경로 자체를 안 타게 함. 앞으로 OneDrive 폴더로 파일 복사하는 코드를 새로 짤 땐 처음부터 이 방식을 쓸 것 — shutil의 metadata-copy/고속-copy 계열 함수는 이모지+OneDrive 조합에서 신뢰 못 함.
