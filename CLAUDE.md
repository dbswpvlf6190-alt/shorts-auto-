...
- **노트북 v2 준비 체크리스트(2026-09-21 기숙사 방문 시 진행 예정)**: ① 노트북에서 `cd C:\shorts_auto` → `git pull` ② 데스크톱의 `credentials/cloudflare.json`을 노트북 같은 경로에 복사(**깃에 올리지 말 것 — 저장소가 공개라 키 노출**; 전송은 사용자 본인만 보는 경로로) ③ `python scripts/check_v2_env.py` 실행 → 모두 OK여야 함(패키지·ffmpeg·목소리 환경·키·이미지 1장 테스트 점검, 비밀값 출력 안 함). 노트북에 키가 없으면 `run_queue.py`가 v2 항목을 **보류(skip)**하고 게시하지 않는다(저품질 영상 공개 방지). 노트북 CPU에 AVX2가 없어 로컬 이미지 생성은 불가 → Cloudflare API만 사용.
