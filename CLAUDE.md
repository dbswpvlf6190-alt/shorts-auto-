...
- **스케줄러에 재시도(Repetition) 추가 (2026-09-02)**: 아침 08:00에 노트북이 막 켜져서 네트워크(와이파이)가 아직 안 붙어있으면 `git pull`이 실패하고, 그날은 그냥 넘어가 버리는 문제가 있었음 — "로컬 연결됐을 때 그날 안에 수행해야지"라는 사용자 피드백으로, 트리거에 30분마다 4시간 동안 반복(Repetition Interval PT30M, Duration PT4H) 설정을 추가함(08:00~12:00 사이 최대 8번 재시도). **`shorts_auto`에서는 안전함** — 매번 새 콘텐츠를 사람이 준비해야 해서 대기열이 보통 비어있고, 이미 처리된 날은 `run_queue.py`가 그냥 "처리할 대기 항목 없음"으로 조용히 끝남. PowerShell로 이 반복 설정을 넣을 때 `New-ScheduledTaskTrigger -Weekly ...`에는 `-RepetitionInterval` 파라미터가 안 먹으므로, `-Once` 트리거를 하나 더 만들어서 그 `.Repetition` 속성만 복사해오는 방식으로 우회해야 함(`$trigger.Repetition = (New-ScheduledTaskTrigger -Once -At ... -RepetitionInterval ... -RepetitionDuration ...).Repetition`).
  - **⚠️ 이 컴퓨터의 다른 프로젝트(`C:\saju-app`, 사주랩)에도 같은 방식을 그대로 적용했다가 실제 사고 발생(2026-09-03)**: 사주랩은 콘텐츠를 미리 대량으로 만들어서 manifest에 쌓아두는 방식이라("하루 실행당 1건"이라는 가정이 스크립트에 박혀있음), 반복 트리거가 30분마다 "다음 미게시 항목"을 계속 찾아 게시해버려서 하루에 릴스 3개·카드뉴스 3개가 연달아 올라간 적 있음(계획은 하루 1개씩). 사주랩 스케줄러에서는 반복 설정을 제거함. **교훈**: 스케줄러 반복(재시도)은 "매번 실행이 안전한 no-op이거나 진짜 새 걸 처리하는" 파이프라인에만 걸 것 — "실행될 때마다 다음 순서 걸 자동으로 하나씩 소비하는" 파이프라인(사전 제작된 backlog/manifest 방식)에는 절대 걸면 안 됨.
...
## 틱톡 배송 파일 바탕화면 자동 복사 (2026-08-26 추가, 2026-08-27/2026-09-03 버그 수정)
`run_queue.py`가 배송 폴더뿐 아니라 `get_tiktok_delivery_path()`로 찾은 위치에도 틱톡 파일을 캡션+해시태그 붙은 파일명 그대로 복사함(사용자가 매번 안 챙겨줘도 되게).
- **주의(인코딩)**: 목적지가 OneDrive로 동기화되는 폴더 + 파일명에 이모지가 있으면, `shutil.copy2`는 물론 `shutil.copyfile`도 큰 파일(영상)에서는 Windows 고속복사 내부 경로를 타면서 `cp949` 인코딩 에러가 남 (작은 파일에선 재현 안 됨). **해결**: `open(...).read()`/`open(...).write()`로 직접 청크 복사(`shutil.copyfileobj`)해서 그 내부 경로 자체를 안 타게 함. 앞으로 OneDrive 폴더로 파일 복사하는 코드를 새로 짤 땐 처음부터 이 방식을 쓸 것.
- **목적지는 바탕화면 루트로 확정 (2026-09-03)**: 바탕화면에 `05_영상_SNS\틱톡 영상\`이라는 정리 폴더가 따로 있고 사용자가 몇 달째 수동으로 뭔가를 모아온 흔적이 있어서, 한때 그 폴더로 자동 리디렉션하도록 바꿨었으나 **사용자가 "아니 바탕화면에 빼줘"라고 명시적으로 정정** — 그 정리 폴더는 이 파이프라인과 무관한 별도 용도이고, 틱톡 배송 파일은 계속 바탕화면 루트에 두는 게 맞음. `get_tiktok_delivery_path()`는 `get_desktop_path()`를 그대로 반환하도록 되돌려놨음. **교훈**: 폴더 구조가 그럴듯해 보여도 사용자한테 직접 확인 안 하고 자동화 목적지를 바꾸지 말 것.

## 채널 SEO (2026-08-29)
- 채널: 구독자 200 / 영상 220개 / 조회수 166,883 (2016년부터 있던 채널, 이 자동화 프로젝트 이전 영상도 다수 포함) — 영상 수·조회수 대비 구독 전환율이 낮은 편, 성장 과제로 남음.
- 채널 키워드가 비어있던 것 발견 → `부동산 경매 전세사기 청약 세금 재테크 부동산정책 권리분석 부동산투자 종부세`로 채움.
- 채널 소개글이 "재테크" 전반의 뭉뚱그린 문구였음(브랜드가 부동산으로 좁혀지기 전 버전) → 부동산 키워드가 명시적으로 들어가게 재작성.
- 이 작업은 `youtube.upload`/`youtube.readonly`(평소 자동 업로드용, token.json)와 별개로 `https://www.googleapis.com/auth/youtube`(채널 정보 수정) 스코프가 필요해서, **일회성으로 별도 인증받고 임시 토큰 파일은 작업 후 바로 삭제함** — 평소 자동화용 token.json 권한 범위는 그대로 최소 유지(최소 권한 원칙).
- 참고: `youtube.upload` 토큰은 테스트 앱이라 7일마다 만료됨 — 2026-08-29에 실제로 만료돼서 재인증함(다음 만료 예상: 2026-09-05 전후, 그 즈음 "유튜브 안 올라가"는 문의 오면 이것부터 의심할 것).
- **재생목록 5개 생성 완료(2026-08-29)**: 부동산 정책·세금(22)/대출·금리(7)/전세·전세사기(9)/경매 노하우·실전(118)/돈 버는 마인드셋(64, 부동산과 직접 관련 없는 옛날 재테크 동기부여형 콘텐츠). 220개 중 193개 배정 완료, 나머지 27개(전부 "돈 버는 마인드셋" 카테고리)는 하루 API 할당량 초과로 못 넣음 — 다음날 할당량 리셋 후 `output/_playlist_buckets.json`(카테고리별 videoId 목록)과 `output/_playlist_ids.json`(재생목록 ID) 보고 이어서 넣을 것. 이 작업은 `youtube` 풀 스코프가 필요해서 매번 별도 1회성 인증 받고 임시 토큰은 작업 후 삭제(위 참고).
- 다음 SEO 후보(미착수): 채널 트레일러 설정.
...
## 이모지 → cp949 콘솔 크래시 (2026-08-30, 근본 원인 발견 및 전면 수정)
**증상**: 13번(전세사기갭투자) 처리 시 "바탕화면 복사 실패", "인스타그램 게시 실패"가 동시에 로그에 남음. 실제로 확인해보니 원인이 하나였음.

**근본 원인**: 캡션에 이모지(😱, 👇 등, BMP 밖 문자)가 들어있는데, `run_queue.py`를 비롯한 여러 스크립트가 `print()`할 때 콘솔 인코딩이 cp949(한글 윈도우 기본값)라서 `UnicodeEncodeError`로 죽음. 문제는 이게 **try/except 안에서 "성공 로그를 남기는 print" 자체가 죽는 것**이라, 실제 작업(파일 복사 등)은 끝났는데 예외가 나서 "실패"로 잘못 기록되는 경우와, 실행 환경(수동/예약작업/백그라운드)에 따라 더 일찍 죽어서 진짜로 작업이 안 끝나는 경우가 섞여 있었음 — 그래서 증상이 들쭉날쭉했음.

**재발 방지 조치 (2026-08-30, 전체 스크립트 일괄 적용)**:
1. `scripts/` 안의 **모든 .py 파일** 맨 위에 `sys.stdout.reconfigure(encoding="utf-8")` 추가 — 콘솔이 cp949든 뭐든 죽지 않게 함. 새 스크립트 만들 때도 이 줄을 습관적으로 넣을 것.
2. `run_queue.py`의 `sanitize_filename()`이 이제 **파일명에서 이모지(BMP 밖 문자)를 아예 제거함** — 그래야 이 클래스의 버그가 다른 곳(아직 못 찾은 곳)에서 또 터져도 파일명 자체엔 영향 없음(2중 방어).
3. 13번 항목의 밀렸던 바탕화면 파일은 수동으로 채워넣음.
- **교훈**: 앞으로 "무슨 무슨 작업이 실패했다"는 로그가 뜨면, 그 작업 자체의 문제인지 아니면 그 작업 뒤에 붙은 로그 출력 자체가 문제인지부터 구분할 것 — 이번처럼 로그 메시지가 원인을 가리는 경우가 있었음.

## 클라우드 자동 대기열 등록 (2026-08-30 도입) — 네트워크 제약 있음
매일 아침 7시(KST)에 claude.ai 클라우드 에이전트가 자동으로 주제를 골라 대기열에 등록하는 예약 루틴이 있음 (`daily.factlab 아침 주제 자동 등록`, trigger_id `trig_01UW8hcHxM3y3TfRnrobD7ow`). 이후 8시에 로컬 컴퓨터가 그걸 영상으로 만들어 게시.
- **2026-09-04(밤, KST 9/5 아침) 실제로 겪은 문제**: 클라우드 샌드박스가 `news.google.com` 등으로의 직접 HTTP 요청(`discover_topics.py`의 `requests` 호출)을 네트워크 정책상 차단(프록시 403) — 후보 0개로 실패, 안전장치대로 아무것도 안 만들고 조용히 종료됨 → 9/5 대기열 항목이 하나 비어서 사용자가 직접 요청해서 데스크톱에서 수동으로 채움(19번).
...
## 2026-09-12 — daily.factlab 채널에 사주랩(saju-app) 교차 홍보 추가
사용자가 본인의 다른 프로젝트(`사주랩`, https://saju-app-three-dusky.vercel.app/ — 생년월일 기반 사주 결과 무료 제공 후 상세 분석 리포트 4,900원에 판매, 대표 윤제필/대국민스토어 명의)를 daily.factlab 유튜브 채널에서 홍보하고 싶다고 요청함. **브랜드 미스매치 우려를 먼저 전달함**(부동산·정책이라는 진지한 정보 채널에 운세 앱을 홍보하면 신뢰도에 안 좋을 수 있다는 우려, 특히 이 채널은 조회수 대비 구독 전환이 이미 병목이라는 기존 분석이 있었음) — 사용자는 위험을 인지하고도 진행을 원해서(영상 콘텐츠 안에 넣는 대신 프로필 링크 정도의 가벼운 형태로) 실행함.
- **한 것**: 채널 소개(`brandingSettings.channel.description`, API로 수정 가능)에 홍보 문구+링크 추가, 그리고 채널 "링크" 버튼(이건 API로 안 되고 YouTube 스튜디오 UI에서만 설정 가능 — Claude in Chrome으로 사용자의 실제 로그인 세션을 통해 직접 클릭해서 등록함, 채널 맞춤설정 > 프로필 > 링크 섹션)에도 등록함. 링크 문구: "🔮 무료 사주 확인 (4,900원 상세분석)".
- **2026-09-12(같은 날) 추가 수정**: 사용자가 "채널 처음 들어왔을 때 '더보기'를 눌러야 보인다"고 지적 — 유튜브 채널 정보 박스의 미리보기는 설명 첫 줄만 보여주는데 원래 부동산 소개 문구가 첫 줄이라 사주 홍보 줄이 가려져 있었음. **사주 홍보 문구를 설명 맨 앞줄로 옮김**(부동산 브랜드 소개보다 앞에 오도록) — 이제 채널 방문 시 클릭 없이 바로 보임. 실제 공개 페이지에서 확인 완료.
- **참고**: 브랜드 정체성보다 노출을 우선한 명시적 선택이므로, 나중에 구독 전환율이 눈에 띄게 나빠지면(주간 리뷰 때 같이 볼 것) 이 위치를 되돌리는 것도 옵션으로 사용자에게 제안할 것.
...
- **사용자가 실제 유튜브 영상 보고 준 피드백 3건 반영**:
  1. 복제 목소리 속도가 원래 채널 속도(1.3배속)를 안 따라가고 있었음(기본값 1.05로만 나감, `--speed` 인자 자체가 안 넘어가고 있었던 버그) → `make_short.py`/`make_platform_variants.py`에 `rate_to_speed()` 추가해서 meta.json의 `rate`("+30%") 값이 실제로 반영되게 수정.
  2. 브롤 이미지 줌인 효과(1.0→1.08 Ken Burns)가 부자연스럽다는 피드백 → 완전히 제거, 이제 고정 이미지 + 크로스페이드만 사용.
...
## 2026-09-16/17 — 인스타그램 게시 이틀 연속 실패, 근본 원인: GCM 인증 프롬프트 무한 대기 + 공유 토큰 만료 (데스크톱 토큰 교체는 2026-09-19에 완료됨 — 아래 "2026-09-19 데스크톱 토큰 교체" 참고)
...
## 2026-09-18 — 영구 비공개 유튜브 영상 목록 (⚠️ 다시 공개로 돌리지 말 것)
아래 영상들은 **의도적으로 비공개** 상태입니다. "이 채널은 항상 공개가 기본"이라는 평소 정책과 다르지만, 예외 상황이니 일반 정책 판단으로 되돌리지 말 것 — 실제로 2026-09-12~18 사이에 이유를 모른 채 공개로 되돌려져서 결함 있는 영상이 조회수 247회를 찍은 사고가 있었음.
- `WO2DB_yq068` — 30번(재건축초과이익환수부담금)의 첫 업로드본. 복제 음성 모델이 만든 오디오가 심하게 깨져서(2026-09-13 사용자 직접 확인) 새로 합성해서 `53A2ihX4ysY`로 교체 업로드함. 이 영상은 그 결함 있는 원본이라 영구 비공개.
- `iSxPjhgNu1g` — 34번(경매고가양극화)의 중복 업로드본 중 조회수가 적은 쪽(6회). GCM 인증 멈춤 버그로 같은 영상이 두 번 올라갔던 사고(같은 날짜 섹션 참고) — `eRkojKpBu6Q`(136회)가 정식본으로 남고 이쪽은 비공개.
- `4rfYoVXMmm4` (2026-09-19 추가) — 36번(토허제실거주유예연장)의 첫 업로드본, 옛 복제 목소리. 사용자가 M3 교체를 확정해서 M3 버전을 `T4sNoQ00elQ`로 교체 업로드하고 이쪽은 영구 비공개. (`input/queue/36_*/youtube_uploaded.txt`도 새 ID로 갱신함.)
- `N-Zrj8-Kp-8` / `9dgEEj0AYZI` / `53A2ihX4ysY` (2026-09-23 추가) — 각각 19·29·30번(전세사기중개사배상·부동산감독원수사권신설·재건축초과이익환수부담금)의 옛 목소리(M3 이전) 원본. M3 목소리로 재렌더해 새 ID(`7dw_GPTq-JU`/`8pkyuSmQPrQ`/`IuS9WG7FBRA`)로 교체 업로드하고 이쪽은 영구 비공개.
- 둘 다 삭제하고 싶으면 사용자가 YouTube 스튜디오에서 직접 삭제하거나, 명시적으로 삭제를 요청할 것(삭제는 자동화 안전장치에 막혀서 매번 사용자 확인이 필요함).
- 이 외에 2026-09-19에 조회수 저조로 비공개 처리한 20편은 `docs/hidden_low_view_videos_2026-09-19.json` 참고.

## 2026-09-19 — 조회수 저조 영상 일괄 비공개 (⚠️ 다시 공개로 돌리지 말 것) + 새 영상 구조 지침 접수
- **유튜브 20편 비공개**: 사용자 요청("전체적으로 조회수 안 나온 건 비공개로")에 따라 기준 **조회수 100 미만 + 업로드 7일 이상**으로 20편을 비공개 처리(옛 영상 17편 + 자동화 영상 3편: 그린벨트 `2vNXHMpTfuI`, DSR `dmiH7YamZuU`, 청약제도 `EUGYGMU_xDs`). 삭제 아님(복구 가능). **전체 ID·제목·조회수·이전 상태는 `docs/hidden_low_view_videos_2026-09-19.json`** — 되돌리려면 그 파일의 id를 `privacy: public`으로. "항상 공개가 기본"이라는 평소 정책으로 자동 복구하지 말 것(9/12~18에 같은 실수가 있었음).
- **인스타그램**: Graph API로 보관/삭제가 불가능해서 사용자가 앱에서 직접 처리(조회수 100 미만 릴스 19개 목록을 파일로 전달함, 32개 중). 조회수는 `graph.instagram.com/{media_id}/insights?metric=views`로 읽힘(토큰 권한 OK).
- **틱톡**: API 연결 자체가 없어(처음부터 수동 업로드) 사용자가 직접 처리.
- **새 영상 구조 지침**: 사용자가 40~50초 세로형·훅 12~15자·결론 먼저·화면 1~2초마다 변화 등 새 제작 지침을 줌 → 원문을 `docs/new_video_structure_spec_2026-09-19.md`에 저장. **아직 파이프라인/클라우드 루틴 프롬프트에는 미반영**(사용자 확인 후 반영할 것). 현재 파이프라인은 정지 카드 3장×5초 + 나레이션 그대로 자막이라 지침의 "1~2초마다 시각 변화·관련 이미지 활용"에는 별도 개발이 필요함.

## 2026-09-19 데스크톱 토큰 교체 완료 + 주말 인스타 누락분 발견
- **데스크톱(DESKTOP-RT3IQO3) `credentials/github_token.txt`를 새 토큰으로 교체.** 이름 `shorts-media-host-desktop`(fine-grained, 저장소 `shorts-media-host` 하나만, Contents 읽기/쓰기, 만료 없음). 값은 사용자가 발급 화면에서 채팅으로 직접 줌(보안상 여기 안 적음). 실제 push·인스타 게시로 동작 확인함. GitHub 토큰 목록에 `desktop-media-host-write`(미사용)도 있음 — 출처 불명, 필요 없으면 사용자가 정리.
- **이 컴퓨터가 처리한 주말 영상은 인스타그램이 전부 조용히 실패해 왔음**(만료 토큰): 19번(9/6), 29번(9/12), 30번(9/13), 36번(9/19). 36번만 M3 목소리로 다시 렌더해서 오늘 게시함. **29·30·19번은 인스타에 아직 없음**(옛 목소리 영상이라 그냥 올리면 안 됨, 사용자 결정 대기). 교훈: "유튜브 올라갔다"만 보고 끝내지 말고 인스타 최신 릴스(`graph.instagram.com/{user_id}/media`)도 같이 확인할 것.
- 실패한 옛 시도 4건(옛 목소리 영상 커밋)이 들어 있던 옛 로컬 저장소는 `~\ShortsAutoRender\media_host_stale_2026-09-19`로 옮겨 보존하고, `media_host`는 새로 clone함(`git reset --hard`는 자동 승인이 막혀서 이렇게 우회). 그 stale 폴더는 지워도 무방.
- **새 도구 `scripts/_rerender_item.py <번호> [--delivery] [--instagram]`**: done.txt가 있는 항목을 새 목소리 등으로 다시 렌더링(+ 배송 파일 교체 + 인스타 게시). 유튜브 업로드는 일부러 안 함(교체는 기존 영상 비공개+재업로드 판단 필요).
- ⚠️ **발견: GitHub 저장소 `shorts-auto-`·`shorts-media-host`·`saju-app`이 전부 PUBLIC 상태**(비인증 API 조회로 200 확인). 이 문서 곳곳의 "비공개 저장소" 표기와 다름. 모든 스크립트/문서/대기열 콘텐츠가 공개돼 있음. 전체 토큰이 커밋된 적은 없음(CLAUDE.md의 34자 잘린 접두어만 있음, 사용 불가) — 다만 공개가 의도한 게 아니라면 사용자가 GitHub 설정에서 `shorts-auto-`를 private으로 바꿔야 함(media-host는 인스타용 raw URL 때문에 public이어야 함). 클라우드 루틴의 저장소 접근은 private이어도 GitHub 연동으로 유지되는지 확인 필요.

## 2026-09-20 — 채널 성장 분석 (상세는 `docs/channel_analysis_2026-09-20.md`, 스크립트 `scripts/analytics/`)
- 핵심: **영상당 도달은 옛 영상과 같은데(15일 조회 중앙 877 vs 838) 시청비율(80%→54%)·좋아요(3배↓)·구독전환(1000뷰당 1.65→0.07, 약 23배↓)이 붕괴**. 자동화 이후 37일간 구독 +4. 시청 이탈은 첫 10초(특히 6~8초)에 집중. 자동화 기간 안에서도 8/30(복제음성)·9/9(방송속보 훅, 길이 58s↑) 이후 단계적으로 악화. 시청자는 45세 이상 92~96%.
- ⚠️ **분석 함정**: YouTube Analytics 일자는 **태평양시** — 창 시작을 공개시각(UTC)-1일로 잡아야 업로드 당일이 안 빠짐(안 그러면 한국 아침 업로드 영상 성적이 1/3로 잘못 나옴, 이번에 실제로 틀린 결론이 나왔다 정정함). per_video 리포트는 상위 ~200편만 반환 → 나머지는 filters로 개별 조회.
- 시청자 이탈 원인 후보 중 "훅-본문 반복"은 검증했으나 무관(p=0.89)이라 기각. 제목 특성·요일·업로드시각은 영향 없음.
- 사용자 새 영상 구조 지침(docs/new_video_structure_spec_2026-09-19.md)은 옛 성공 포맷(주제 관련 이미지 + "~해요" 체 + 30~40초)과 방향이 일치함. 반영은 사용자 확인 후.

## 2026-09-20 — 새 영상 포맷 시제품(재산세) + 로컬 AI 이미지 생성 (⚠️ 시제품 단계, 파이프라인 미반영)
- 배경: 채널 분석(위 섹션)에서 옛 성공 영상 = **AI 이미지(주제 관련 실사풍) + "~해요" 체 + 30~40초**로 확인, 사용자가 "옛날 영상은 AI로 만들었다"고 확인. 새 구조 지침(`docs/new_video_structure_spec_2026-09-19.md`)과 방향 일치.
- **로컬 이미지 생성 구축 완료**: `vendor/imagegen/venv`(CPU PyTorch + diffusers, vendor/는 git 미추적 → 재설치 필요) + `scripts/imagegen_local.py`(Lykon/dreamshaper-8 + LCM-LoRA 4단계, 512x768, 상업 이용 가능 OpenRAIL 라이선스). **이미지당 약 50초(CPU), 모델 로드 첫 회 132초/이후 16초.** 이 데스크톱: GTX 1060 **3GB**(VRAM 작아 CPU 사용), RAM 16GB, i7-6700. 노트북(i5-3230M, AVX2 없음)은 사실상 불가로 추정(미검증).
- 품질: 풍경/사물 분위기 컷은 좋음, **글씨 든 물체(계약서·달력)는 글자 뭉개짐 → 날짜·숫자는 반드시 우리가 PIL로 그려 얹을 것**, 사람은 프롬프트대로 안 나오는 경우 있음(얼굴/손 회피 권장).
- 시제품: `output/proto_jaesan/`(build.py, 6장 이미지, proto_jaesan_v2.mp4 38.9초 12장면, 로컬 전용). 총 계산시간 약 6분(이미지 5.2분 + 목소리 17초 + 렌더 24초).
- 무료 이미지 API 조사 결과: Pollinations는 API 키 필요로 바뀜(키 없으면 500), **Gemini 이미지는 공식 가격표상 무료 등급 없음**(웹 기사의 "무료 500장" 주장은 틀렸음), Cloudflare Workers AI FLUX는 무료 하루 1만 뉴런(≈170장)이나 **계정 이메일 인증이 안 끝나 API 토큰 생성이 막힌 상태**(계정은 dbswpvlf6190@gmail.com으로 가입됨, 중복 가입 금지, 인증 후 Workers AI 템플릿 토큰 발급 재개 가능).

## 2026-09-20 (이어서) — Cloudflare Workers AI(FLUX) 연결 완료, 이미지 생성 표준으로 채택 후보
- 이메일 인증 완료 후 **Workers AI 토큰 발급 성공**. 자격증명: `credentials/cloudflare.json`(account_id + api_token, git 제외, **컴퓨터마다 따로 넣어야 함 — 노트북엔 아직 없음**). 토큰 권한은 이 계정의 Workers AI 읽기/편집만. 결제 수단 미등록이라 한도 초과 시 과금이 아니라 차단.
- `scripts/imagegen_cloudflare.py`: `@cf/black-forest-labs/flux-1-schnell`, 표준 라이브러리만 사용, 장당 약 1.5~3초, 출력은 **1024x1024 고정**(width/height 파라미터 미지원 — 400 에러 확인). 9:16은 가운데 세로로 잘라 1.875배 확대(시제품 build.py의 cover()가 이렇게 처리). 무료 한도 하루 10,000 뉴런 ≈ 이미지 약 170장(1장 ≈ 58뉴런).
- 로컬(dreamshaper/LCM, 장당 50초, 512x768) 대비 훨씬 선명·빠름 → **노트북도 API 호출만 하면 되므로 "노트북은 이미지 생성 불가" 문제 해소**(토큰 파일만 복사하면 됨). 클라우드 루틴 샌드박스는 외부 HTTP 차단이라 거기서는 못 부름 → 이미지는 로컬 파이프라인 실행 시점에 생성.
- 품질 주의: 간판·라벨·지폐 같은 **글자가 있는 사물에는 가짜 글자가 생김**(우편함 "MIBSTTEE", 지폐 "KCONA/1300"). 프롬프트에서 간판/지폐/문서 클로즈업 피하고 no text를 넣되 완벽하진 않음 → 필요하면 살짝 크롭·어둡게.
- **시제품 v4(2026-09-20)**: v3 대비 = 금액 예시(재산세 한 해 200만 원 가정, "예시 금액" 표기), 손실·기한 훅("판 집인데 재산세 200만 원?" + 9/30 넘기면 3%), 중간 반전 질문("딱 6월 1일에 넘기면?" → "산 사람이에요"), 출처 라벨, 공유("집 파는 지인께 보내주세요")·댓글("몇 월 며칠에 파셨어요?") 엔딩, 자막 팝인·숫자 카운트업·배지 애니메이션(이미지 줌 없음), numpy 합성 효과음(컷 전환 whoosh/임팩트/딩), 글자 없는 이미지 7장(Cloudflare). 빌더 `output/proto_jaesan/build_v4.py`(로컬 전용), 총 빌드 약 70초. 미검증: 실제 청취 품질, 200만 원 가정이 오해를 부르는지, "계약서 정산 특약" 같은 실무 관행은 출처 미확인이라 뺐음.

## 2026-09-20 — v2 영상 포맷 파이프라인 통합 (사용자 "전면 전환" 결정)
- 사용자가 v4 시제품을 보고 **전면 전환**을 선택. `meta.json`에 `"format":"v2"` + `"scenes"`가 있으면 `run_queue.py`가 `scripts/make_video_v2.py`로 렌더(대본·장면 스키마·이미지 프롬프트 규칙은 `docs/script_template_v2.md`). `format`이 없으면 기존 방식으로 렌더 → **롤백 = 루틴이 format을 안 쓰게 되돌리기**.
- v2 렌더 산출물: `platform/youtube.mp4`, `tiktok.mp4`, `thumb.png`(유튜브 썸네일로 사용), `v2_report.json`. v2 항목은 유튜브 업로드에 `--synthetic`(containsSyntheticMedia) 전달. API가 값을 받아도 읽기값이 None으로 나온 적 있어 **첫 실제 v2 업로드 뒤 스튜디오에서 "변형된 콘텐츠" 표시가 실제로 붙었는지 확인**(업로드 로그에 확인 줄이 찍힘).
- 이미지는 렌더 시점에 Cloudflare로 생성(credentials/cloudflare.json 필요, **노트북엔 수동 복사 필요**). 실패하면 그라데이션 배경으로 대체되어 영상은 나옴.
- 통합 테스트(격리 디렉터리, 업로드 호출은 가짜): v2 분기 렌더 → --synthetic·thumb 전달 → 인스타 호출 → 체크포인트 마커 확인 완료.
- 남은 일: 클라우드 루틴 프롬프트를 v2 meta(scenes+image_prompt)를 쓰게 수정(사용자 승인 필요), 노트북 cloudflare.json 배치, 주간 지표 점검(풀링 참여 조회 비율·10초 유지율·좋아요/1000·구독/1000).
- **✅ 노트북 v2 준비 완료(2026-09-21 06:40 KST)**: 노트북에서 git pull → `credentials/cloudflare.json` 배치(사용자가 준 Downloads 파일 복사, git 제외 확인) → 시스템 Python에 `soundfile`이 없어서 `pip install soundfile` → `check_v2_env.py` 전부 통과(Cloudflare 이미지 1장 테스트 포함). 이제 노트북도 v2 항목을 보류 없이 렌더한다. 아직 남은 확인: 9/21 07:06 KST 첫 v2 클라우드 루틴 결과(`39_*_v2`, `validate_v2_meta.py`).
- **노트북 v2 준비 체크리스트(2026-09-21 기숙사 방문 시 진행 예정, 위에 완료 처리됨)**: ① 노트북에서 `cd C:\shorts_auto` → `git pull` ② 데스크톱의 `credentials/cloudflare.json`을 노트북 같은 경로에 복사(**깃에 올리지 말 것 — 저장소가 공개라 키 노출**; 전송은 사용자 본인만 보는 경로로) ③ `python scripts/check_v2_env.py` 실행 → 모두 OK여야 함(패키지·ffmpeg·목소리 환경·키·이미지 1장 테스트 점검, 비밀값 출력 안 함). 노트북에 키가 없으면 `run_queue.py`가 v2 항목을 **보류(skip)**하고 게시하지 않는다(저품질 영상 공개 방지). 노트북 CPU에 AVX2가 없어 로컬 이미지 생성은 불가 → Cloudflare API만 사용.
- **2026-09-20 오후**: `scripts/validate_v2_meta.py`(표준 라이브러리만, 클라우드에서도 실행 가능) 추가 — v2 meta 규칙 위반을 ERROR/WARN으로 알려줌. 37번 재산세를 v2로 재제작해 `37_재산세2기분6월1일기준_v2`로 게시(유튜브 `_w4Bjqk7uEo`, 인스타 게시, 틱톡 파일 배송됨). **옛 37번 유튜브 `KuwwtfbNCYM`은 비공개로 전환됨(다시 공개 금지)**. 인스타/틱톡의 옛 37번 게시물은 사용자가 직접 정리(인스타는 API로 보관/삭제 불가). 업로드 후에도 containsSyntheticMedia 읽기값은 여전히 None → 스튜디오 수동 확인 필요.
- **클라우드 루틴 프롬프트를 v2용으로 교체 완료(2026-09-20 12:33 UTC 이전, 사용자 승인 후 RemoteTrigger update 성공)**: 전문은 `docs/routine_prompt_v2.md`. 첫 v2 실행은 2026-09-20 22:06 UTC(9/21 07:06 KST) — **다음 세션에서 `list_runs`/`get_run_log`와 새로 생긴 `39_*_v2/meta.json` + `validate_v2_meta.py` 결과를 반드시 확인할 것.** 대기열 순서상 `38_비거주1주택종부세원상복구`(옛 방식)가 먼저 처리되므로 자동 v2 첫 영상은 9/22 예정. 롤백하려면 이 프롬프트를 옛 것(git 이력의 docs 아님 — 이전 프롬프트는 이 세션 기록에만 있음, 필요 시 script.txt+images 방식 지시로 재작성)으로 되돌림.

## 2026-09-21 — 인스타 게시 실패 원인 #3: media_host push가 "fetch first"로 거부됨 (수정 완료)
노트북이 오늘 38·39번을 처리했는데 인스타만 둘 다 실패. 원인: `shorts-media-host`를 노트북·데스크톱이 번갈아 쓰는데(주말에 데스크톱이 올림), `publish_instagram.py`가 push 전에 최신을 안 받아와서 non-fast-forward로 거부됨. (9/16-17의 GCM 멈춤과는 별개 원인 — 그 방어 덕에 이번엔 에러 원문이 로그에 그대로 읽혔음.) **수정**: `commit_and_push()`가 push 전에 `git pull --no-rebase`로 합치고, 거부되면 한 번 더 합쳐 재시도하며, 이미 커밋된 같은 파일 재시도는 커밋을 건너뜀(예전엔 "nothing to commit"으로 실패). saju-app의 `publish_instagram.py`/`publish_carousel.py`에도 동일 적용. 38·39번 인스타는 수정된 코드로 수동 catch-up 게시 완료.
- 39번(첫 자동 v2)은 클라우드 루틴이 07:07에 정상 등록, 검증 오류 0/경고 3(voice에 `%` — 읽기 표기로 풀어 쓰라는 경고를 루틴이 무시함, 실제 음성에서 어색한지 확인 필요), 노트북이 렌더·유튜브 업로드(JD3rQ6n04sM) 완료. 스튜디오에서 "변형된 콘텐츠" 표시 수동 확인은 아직.

## 2026-09-21 — 업로드/게시 결과를 사용자 폰으로 푸시 보고 (ntfy)
`scripts/notify.py`(표준 라이브러리만)가 `credentials/ntfy.json`의 토픽으로 ntfy.sh에 푸시를 보냄. `run_queue.py`가 항목 처리 끝에 `report_upload()`로 "✅ 업로드 완료 · 제목/유튜브 링크·공개여부/인스타 성공·실패/처리 컴퓨터"를 보내고(인스타 실패·비공개면 ⚠️ 우선순위 높임), 어떤 단계든 예외로 죽으면 "❌ 처리 실패"를 보냄. 사주랩(saju-app)의 릴스·카드뉴스 러너도 같은 방식(게시 완료 시 고정댓글 문구까지 푸시에 포함, 실패 시 ❌).
- **⚠️ `credentials/ntfy.json`은 git 제외(토픽 이름이 비밀번호 역할 — 이 저장소는 공개라 토픽을 여기 적지 않음). 컴퓨터마다 같은 파일이 필요**: 이 노트북엔 생성 완료. 데스크톱엔 없으면 푸시가 조용히 생략됨(파이프라인은 정상) → 데스크톱에서 사용자에게 토픽을 물어 `{"topic": "...", "server": "https://ntfy.sh"}`로 만들 것. 폰: ntfy 앱 설치 후 그 토픽 구독.
- 테스트: `python scripts/notify.py --test`. 알림 전송은 8초 타임아웃·예외 무시라 실패해도 파이프라인엔 영향 없음.

## 2026-09-23 — 인스타 미게시분(19·29·30번) 캐치업 + 유튜브도 M3로 전량 교체 + 데스크톱 ntfy 연결
- **19·29·30번 인스타그램 게시 완료**: 9/17 GCM 무한대기 버그로 세 항목 모두 유튜브만 올라가고 인스타는 조용히 실패했었음(원인은 위 "9/19 데스크톱 토큰 교체" 섹션). `scripts/_rerender_item.py <번호> --instagram`으로 각각 다시 렌더(현재 기본 목소리 M3 적용) 후 인스타에 게시, Graph API로 3건 다 확인됨(2026-09-23 10:29~10:34 UTC).
- **유튜브도 같은 M3 렌더로 교체 업로드 완료(사용자 확인 후 진행)**: 옛 목소리 원본 3개를 비공개 전환하고, 새 영상을 공개로 재업로드(`input/queue/<번호>_*/youtube_uploaded.txt`도 새 ID로 갱신).
  - 19번(전세사기중개사배상): 옛 `N-Zrj8-Kp-8`(비공개) → 새 `7dw_GPTq-JU`
  - 29번(부동산감독원수사권신설): 옛 `9dgEEj0AYZI`(비공개) → 새 `8pkyuSmQPrQ`
  - 30번(재건축초과이익환수부담금): 옛 `53A2ihX4ysY`(비공개) → 새 `IuS9WG7FBRA`
  - 위 "2026-09-18 영구 비공개 유튜브 영상 목록"에도 이 3개를 추가로 등록할 것(다시 공개로 돌리지 말 것).
- **데스크톱(DESKTOP-RT3IQO3)에 ntfy 알림 연결**: `credentials/ntfy.json` 생성, 테스트 알림 전송 성공. 이제 데스크톱이 처리한 항목도 폰 푸시가 감(노트북과 별개 토픽).
- 데스크톱 v2 환경 점검(`check_v2_env.py`) 전부 통과 확인, 유튜브 token.json도 살아있음(재인증 불필요).

## 2026-09-22/23 — 추석 연휴 데스크톱 인수인계 (⚠️ 데스크톱에서 확인할 것)
- **유튜브 토큰 재인증(9/22, 이 노트북에서만)**: 7일 주기 테스트앱 재인증 만료로 40번 업로드가 오늘 두 번 실패(RefreshError)한 뒤 재인증 완료, 이후 정상 처리됨. **`credentials/token.json`은 컴퓨터마다 로컬 파일이라 데스크톱은 별개** — 데스크톱이 7일 넘게 안 돌았다면 거기서도 같은 이유로 실패할 수 있음. 증상: `❌ daily.factlab 처리 실패` 푸시(또는 `output/queue_log.txt`에 `RefreshError`/`invalid_grant`) → `python scripts/_reauth_youtube.py` 실행해서 브라우저로 재인증.
- **업로드 결과 폰 푸시(ntfy, 9/21 도입)가 아직 이 노트북에만 연결돼 있음**: `credentials/ntfy.json`이 컴퓨터마다 로컬(git 제외)이라, 데스크톱에서 처리해도 지금은 폰에 알림이 안 감(파이프라인 자체는 정상 동작 — 알림 실패는 조용히 무시됨). **데스크톱에서 Claude에게 "ntfy 설정해줘"라고 하면, 토픽 값(사용자가 알고 있음 — 보안상 git에는 안 적음)을 물어서 `{"topic": "...", "server": "https://ntfy.sh"}`로 `credentials/ntfy.json`을 만들어줄 것.** 테스트: `python scripts/notify.py --test`.
- **훅 릴스(원석형) 중복 게시 아직 미정리**: saju-app OPS_NOTES.md 9/21 섹션 참고 — 2026-09-18 구버전 인스타 게시물(media_id `18086839340682411`)이 아직 삭제 안 됨(사용자가 앱에서 직접 삭제 예정이었음). 새 세션이 또 올리지 말 것.
- 그 외 최근 변경(media_host push 충돌 방지, v2 영상 포맷, Cloudflare 이미지 생성)은 이 파일 위쪽 섹션들 참고 — 노트북 세팅은 9/21에 완료됐고 데스크톱은 이미 v2 렌더 경험이 있음(37/39번을 데스크톱이 만든 기록 없음 — 실제로는 이 노트북이 렌더함, 데스크톱은 `credentials/cloudflare.json` 보유 여부를 `python scripts/check_v2_env.py`로 확인할 것).
...
- 2026-09-17: **인스타그램 게시 이틀 연속 실패 원인 규명·수정** — GCM 인증 프롬프트 무한 대기 + saju-app과 공유하던 media_host 토큰 만료가 원인. git 호출에 타임아웃/프롬프트 차단 추가, 새 fine-grained PAT 발급해 3개 저장소(saju-app/saju-media-host/shorts-media-host)에 반영. (데스크톱 컴퓨터의 `credentials/github_token.txt`는 2026-09-19에 교체 완료.)
